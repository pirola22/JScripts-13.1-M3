	...
	class mws473_B {
		...
		debug.WriteLine('------------------------------------------------------------------------start---------------------');
			try {
		 	...
		 	calc(0, rows.Count - 1, 'RMQT');	
			//	calc(0, rows.Count - 1, 'NSTT');
				makeLb(0, rows.Count - 1 )				
				//end logic
				oldCount = newCount = rows.Count;
				debug.WriteLine('------------------------------------------------------------------------end calc---------------------');
		...
		debug.WriteLine('------------------------------------------------------------------------Calc---------------------');
			try {
				...
				for (var i = from; i <= to; i++) { //strt for
					...
					var UM = 'LBS';//row[listControl.GetColumnIndexByName('UNMS')]; 
					if (UM == 'LBS') {
		...
		debug.WriteLine('------------------------------------------------------------------------OnScrollChanged---------------------' +e.VerticalChange);
		...
		function OnRequestCompleted(sender : Object, e : RequestEventArgs) {
			debug.WriteLine('------------------------------------------------------------------------On Request completed---------------------');
			try {
