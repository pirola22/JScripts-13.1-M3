...
package MForms.JScript {
	class pms040 {
		...
		function MyStart() {
			try {
				...
				if (hasHeader(sort)) {
					...
					block = ScriptUtil.FindChild(content, "LBL_L27T5").Content;
					var UOM = block.Text;
					if (UOM == 'LBS') {

						var field = new TextBox();
						field = ScriptUtil.FindChild(content, "WHORQA");
						textCalc(field);
					}
				}
				calc(0, rows.Count - 1, 'CQOH');
				calc(0, rows.Count - 1, 'NTQT');
				calc(0, rows.Count - 1, 'RQQT');
				calc(0, rows.Count - 1, 'REQT');
				calc(0, rows.Count - 1, 'ALQT');
				calc(0, rows.Count - 1, 'RBQT');
		...
		function hasHeader(sort : String) : boolean {
			if (sort == 1) {
				return true;
			} 
			if (sort == 2) {
