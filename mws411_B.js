	...
	class mws411_B {
		...
		function MyStart() {
			try {
				...
				// logic
				calc(0, rows.Count - 1, 'DLQT');
				calc(0, rows.Count - 1, 'PLQT');
				calc(0, rows.Count - 1, 'ORQT');
		...
		function makeLb(from, to) {
			try {
				...
				for (var i = from; i <= to; i++) {
					...
					var UM = row[listControl.GetColumnIndexByName('ALUN')];
					if (UM == 'KG') {
						var newItems = new String[row.Items.length + 1];
						row.Items.CopyTo(newItems, 0);
						newItems[listControl.GetColumnIndexByName('ALUN')] = 'LB';
					} else {
						var newItems = new String[row.Items.length + 1];
						row.Items.CopyTo(newItems, 0);
					}
		...
		function OnScrollChanged(sender : Object, e : ScrollChangedEventArgs) {
			if (e.VerticalChange > 0) {
				...
				if (diff > 0) {
					...
					// logic Jean 1-14-15
					calc(fromRow, toRow, 'DLQT');
					calc(fromRow, toRow, 'PLQT');
					calc(fromRow, toRow, 'ORQT');
